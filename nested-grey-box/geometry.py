...
# from NestedGreyBox.climate import Energy
...
class NGBShoebox:
    ...
    def _get_envelope_dat(self):
        ...
        self.SHGCcoeff_dif = [[17.0, 0.93], [41.0, 2.03], [55.0, 1.87], [67.0, 1.6], [78.0, 1.36], [90, 1.1]] #[[18.0, 0.83], [43.0, 1.65], [57.0, 1.58], [68.0, 1.4], [79.0, 1.23], [90, 0.98]]
        self.SHGCcoeff_dir = [[34.0, 0.94], [42.0, 0.93], [51.0, 0.89], [61.0, 0.82], [70.0, 0.67], [90, 0.49]]
    ...
    def _gen_test_schedules(self):
        ...
        ### Occupancy Schedule
        sch_occ = []
        for i in range(1,7): sch_occ.append((i,0))
        sch_occ.append((7,0.1))
        sch_occ.append((8,0.2))
        for i in range(9,13): sch_occ.append((i,0.95))
        sch_occ.append((13,0.5))
        for i in range(14,18): sch_occ.append((i,0.95))
        sch_occ.append((18,0.3))
        sch_occ.append((19,0.1))
        sch_occ.append((20,0.1))
        for i in range(21,25): sch_occ.append((i,0.05))
    ...
    @classmethod
    def save_idf_from_template(cls, name, template, zone_data, wwr_map,
