...
class RadFacade:
    ...
    def __init__(self, clim_obj, solar_position_method="nrel_numpy"):
        ...
        self.location = Location(
            self.clim_obj.latitude,
            self.clim_obj.longitude,
            name=self.clim_obj.city,
            altitude=self.clim_obj.altitude,
            tz=pv_tz,
        )
...
class ShoeboxEnergy:
    ...
    def calc_solar_gains(self):
        ...
        self._dirrad_facade, self._difrad_facade = (
            self._rad_obj.get_irradiance_for_surface(
                tilt=90, surface_azimuth=self.orientation
            )
        ...
        AOI = [
            irradiance.aoi(0, self.orientation, z, a)
            for z, a in zip(
                self._rad_obj.solar_position["zenith"],
                self._rad_obj.solar_position["azimuth"],
            )
        ]
        AOI = np.array(AOI)
        AOI[AOI < -90] = 0
        ...
        for a in range(len(self.shoebox.SHGCcoeff_dir)):
            shgc_tempDir[np.where(AOI < self.shoebox.SHGCcoeff_dir[a][0])] = (
                self.shoebox.SHGCcoeff_dir[a][1] * shgc__79_dir
            )
        ...
        for a in range(len(self.shoebox.SHGCcoeff_dif)):
            shgc_tempDif[np.where(AOI < self.shoebox.SHGCcoeff_dif[a][0])] = (
                self.shoebox.SHGCcoeff_dif[a][1] * shgc__79_dif
            )
        ...
        if self.SHGCbool == False:
            solarGain = (
                self.dir_sol_gain * self.shoebox.SHGC * temp_shading
                + self.dif_sol_gain * self.shoebox.SHGC * temp_shading
            )
        else:
            solarGain = (
                self.dir_sol_gain * shgc_tempDir * temp_shading
                + self.dif_sol_gain * shgc_tempDif * temp_shading
            )
    ...
    def calc_internal_gains(self):
        ...
        self.LightGain_hr = (
            self.shoebox.LPD * self.shoebox.lgtSch[:, 1]
        )  # *LPD_floorPercentage
        self.EquipGain_hr = self.shoebox.EPD * self.shoebox.eqpSch[:, 1]
        self.PplGain_hr = (
            self.shoebox.PPD
            * self.shoebox.occSch[:, 1]
            * self.shoebox.width
            * self.shoebox.length
        )
